        ...
        if isinstance(data,pd.DataFrame):
            ...
            for k in range(1,len(data[:,0])-1):
                ...
                if (diff2>0) and (diff1<=0):
                    ...
                    peak_val = np.append(peak_val,data[k,ind])

        #contingency for objects that are not pandas
        else:
            if i>1000:
                print (i,np.mean(data[ind]))
            for k in range(1,len(data[0])-1):
...
def RX_partitioning_replicates_extended(data_arr,ind,perc,Skip=[],extension=0,Cap=None,ll=0):
    ...
    if extension>0:
        ...
        Issues,pos_out1,vals_out1=sm_filter(pos_out,vals_out)
        
        popt, pcov = curve_fit(TM_func, pos_out1,vals_out1,maxfev=8000)
    ...
    for i in range(len(data_arr)):
        ...
        if i in Skip or i in poor_traces:
            data_out.append(np.array([]))
            data_out2.append(np.array([]))
            data_out3.append(np.array([]))
            peaks_TM2.append(np.array([]))
            continue
        ...
        if extension<0:
            ...
            if i in Issues:
                peaks = peaksTM[i][:1]
                if extension>0:
                    for ppp,pt in enumerate(added_peak_diffs2):
                        if ppp<len(marker_diffs)-1:
                            peaks=np.append(peaks,peaks[-1]+pt*(marker_diffs[ppp+1]/10))
                        else:
                            peaks=np.append(peaks,peaks[-1]+pt)
            else:
                peaks = peaksTM[i]
...
def RX_partition_realignment_extended(partition,inds,data_arr1,fl=None,corr_b=0.7,inspect=1000,Cap=None,perc=0.25,tm=0,ll=0,marker_set='ROX'):
    ...
    labs=list(map(chr, range(65, (65+len(inds)))))
        ...
        if np.any(lens<bins):
            ...
            if len(inds)==2:
                ...
                if np.all(np.array(lens1)>0):
                    ...
                    for fg in range(len(bars)):
                        bar_lens.append(len(bars[fg]))
