...
def deriv1_step2(array):
    n=len(array)
    d=np.zeros(n)
    d[0]=array[2]-array[0]
    d[1]=array[3]-array[0]
    d[n-1]=array[n-1]-array[n-3]
    d[n-2]=array[n-1]-array[n-4]
    for j in np.arange(2,n-2):
        d[j]=(array[j+2]-array[j-2])/2
    return d
...
def fitLinear(x,y,NData):
    ...
    for i in range(NPoint-1):
        x1=np.array([int(x[i]),int(x[i+1])])
        y1=np.array([int(y[i]),int(y[i+1])])
        coeff=np.polyfit(x1,y1,1)
        poly=np.poly1d(coeff)
        xNew=np.arange(int(x[i]),int(x[i+1]))
