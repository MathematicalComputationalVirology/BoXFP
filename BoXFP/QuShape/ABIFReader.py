...
if __name__ == "__main__":
    ...
    # get data as list
    reaction_channel_1 = reader.getData('DATA',1)
    sequencing_channel_1 = reader.getData('DATA',2)
    reaction_channel_2 = reader.getData('DATA',3)
    sequencing_channel_2 = reader.getData('DATA',4)
    
    # produce CSV data
    f = open(sys.argv[1].replace('.fsa', '_raw.csv'), 'w')
    f.write('Position,ReactionChannel#1,SequenceChannel#1,ReactionChannel#2,SequenceChannel#2\n')
    for position in range(len(reader.getData('DATA',1))):
        f.write('%d,%d,%d,%d,%d\n' % (position+1,
                                      reaction_channel_1[position],
                                      sequencing_channel_1[position],
                                      reaction_channel_2[position],
                                      sequencing_channel_2[position]))
    f.close()
